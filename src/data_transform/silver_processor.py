        ...
        for col in df.columns:
            if "date" in col.lower():
                try:
                    ...
                    logger.debug(
                        f"Coluna convertida para datetime: {col} | Tipo final: {df[col].dtype}"
                    )
                except Exception as e:
                    logger.warning(f"Falha ao converter {col}: {e}")
            logger.debug(
                f"Entrou na conversão de data para {col} com tipo {df[col].dtype}"
            )
        return df
    ...
    def handle_nas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, pct in na_pct.items():
            if pct == 0:
                ...
                logger.warning(
                    f"Muitos nulos em {col} ({pct:.1f}%) — mantido como está"
                )
        return df
    ...
    def process_file(self, df: pd.DataFrame, base_name: str) -> pd.DataFrame:
        ...
        if "date" in df.columns and pd.api.types.is_datetime64_any_dtype(df["date"]):
            if df["date"].dt.hour.nunique() > 1:
                ...
                logger.debug(
                    f"Numero de dias duplicados : {df['date'].duplicated().sum()}"
                )
    ...
    def process_all_by_city(self):
        ...
        for base_folder in sorted(self.bronze_path.iterdir()):
            ...
            for parquet_file in sorted(base_folder.glob("*.parquet")):
                ...
                if not city:
                    logger.warning(
                        f"Cidade não identificada no nome do arquivo: {parquet_file.name}"
                    )
                    continue
                ...
                if processed_df.empty:
                    logger.warning(
                        f"Arquivo processado resultou vazio: {parquet_file.name}"
                    )
                    continue
