...
class BronzeProcessor:
    ...
    def __init__(self, raw_data_path: str, bronze_output_path: str, mapping_file_path: str):
        """
    ...
    def save_to_parquet(self, df: pd.DataFrame, output_path: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"Arquivo salvo: {output_path.name} | Size: {file_size_mb:.2f} MB")
            return True
            ...
            if base_folder.is_dir():
                ...
                for pq_file in parquet_files[:1]:  # Testar apenas o primeiro
                    try:
                        ...
                        logger.debug(f"Validação OK: {pq_file.name} - Shape: {df.shape}")
                    except Exception as e:
