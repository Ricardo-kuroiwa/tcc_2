...
class DisasterAnalysis:
    ...
    def show_event_type_over_years(self, df):
        ...
        pivot = df.pivot_table(
            index="year", columns="eventType", aggfunc="size", fill_value=0
        )
        ...
        plt.tight_layout()
        utils.save_plot(
            "base_disaster/event_type_over_years",
            f"event_type_over_years_{self.city}.png",
        )
    ...
    def show_disaster_over_years(self, df):
        ...
        all_years = pd.Series(
            range(df["year"].min(), df["year"].max() + 1), name="year"
        )
        ...
        window_size = 3
        df_grouped = utils.add_moving_average(
            df_grouped, column="count", window=window_size
        ...
        ax.bar(
            df_grouped["year"], df_grouped["count"], color="skyblue", label="Desastres"
        )
        ax.plot(
            df_grouped["year"],
            df_grouped["count_moving_average_sma"],
            color="red",
        ...
        ax.plot(
            df_grouped["year"],
            df_grouped["count_moving_average_ema"],
            color="green",
            label="Média Móvel expodencial (EMA)",
            linewidth=2.5,
            linestyle="--",
        )
        ...
        # Salvar o gráfico
        utils.save_plot(
            "base_disaster/event_over_years", f"event_over_years_{self.city}.png"
        )
    ...
    def show_disaster_by_year(self, df):
        ...
        utils.save_plot(
            "base_disaster/disaster_by_year", f"disaster_by_year_{self.city}.png"
        )
    ...
    def show_disaster_by_event_type(self, df):
        ...
        utils.save_plot(
            "base_disaster/disaster_by_event_type",
            f"disaster_by_event_type_{self.city}.png",
        )

    def show_count_disaster_by_city(self, dfs_by_city):
        disaster_count = {city: df.shape[0] for city, df in dfs_by_city.items()}
        df = pd.DataFrame.from_dict(
            disaster_count, orient="index", columns=["count"]
        ...
        plt.ylabel("Cidade")
        utils.save_plot(
            "base_disaster/count_disaster_by_city", "count_disaster_by_city.png"
        )
    ...
    def show_disaster_by_season(self, df):
        ...
        plt.figure(figsize=(10, 5))
        ax = sns.barplot(
            data=disaster_by_season, x="season", y="count", hue="season", palette="hls"
        )
        ...
        utils.save_plot(
            "base_disaster/disaster_by_season", f"disaster_by_season_{self.city}.png"
        )
