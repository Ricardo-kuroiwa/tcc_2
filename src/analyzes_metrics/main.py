...
def show_compare_balancing_by_city(
    df: pd.DataFrame, path: str, metric: str = "f1_score"
):
    """
    ...
    for city in distinct_cities:
        ...
        for model_name, group in df_grouped:
            ...
            ax = sns.barplot(
                x=group["balancing_method"], y=group[metric], palette="hls"
            )
            ...
            ax.set_title(
                f"Comparação de {metric} por cidade: {city} - Modelo: {model_name}"
            )
            ax.set_xlabel("Metodo de Balanceamento")
            ax.set_ylabel(f"Metrica {metric}")
            utils.save_plot(
                f"analyzes_metrics/compare_balancing_by_city/{path}/",
                f"{city}_{model_name}_{metric}.png",
            )


def show_scatter_performance(
    df, base: str, metric="f1_score", time_col="training_time"
):
    """
    ...
    for city in distinct_cities:
        ...
        plt.ylabel(metric)
        plt.legend(
            title="Método de Balanceamento", bbox_to_anchor=(1.05, 1), loc="upper left"
        )
        plt.tight_layout()

        # Salvar o gráfico usando a função utilitária (assumindo que ela existe)
        utils.save_plot(
            f"analyzes_metrics/scatter_performance/{base}",
            f"{city}_scatter_performance_{metric}.png",
        )
        plt.close()
...
if __name__ == "__main__":
    ...
    for file in files_in_folder:
        ...
        show_scatter_performance(
            df, os.path.splitext(file)[0], metric="recall", time_col="training_time"
        )
