...
def train_decision_tree(
    X_train, y_train, X_test, y_test, collumn_target, base, city, balancing_method=None
):
    try:
        ...
        with mlflow.start_run(run_name=f"Decision Tree Classifier Optuna - {city}"):
            ...
            balanceador = (
                balancing_methods[balancing_method](study.best_trial)
                if balancing_method
                else None
            )
            X_train, y_train = (
                balanceador.fit_resample(X_train, y_train)
                if balanceador
